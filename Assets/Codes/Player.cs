...
public class Player : MonoBehaviour
{
    ...
    {
        if (collision.CompareTag("EnemyBullet")) // 적에 닿을 경우 데미지를 받음
        {
    ...
    void Damage(int damage) // 캐릭터가 데미지를 받음
    {
        hp -= damage;
    ...
    public void ApplySlow(float slowAmount, float duration) // 이동 속도 디버프 효과
    {
        StartCoroutine(SlowCoroutine(slowAmount, duration));
    }
    IEnumerator SlowCoroutine(float slowAmount, float duration)
    {
        speed = baseSpeed * (1f - slowAmount); // 이동 속도를 디버프 효과 만큼 줄임
        yield return new WaitForSeconds(duration); // 지속 시간 이후에 기본 이동 속도로 돌아옴
        speed = baseSpeed;
    }
}
