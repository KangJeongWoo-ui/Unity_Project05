...
public class Spawn : MonoBehaviour
{
    ...
    public GameObject iceLance; // 얼음창
    public GameObject safeZone; // 안전 지대
    ...
    {
        GameObject line = Instantiate(meteorLine, spawnPoint[index].position + new Vector3(0, -6, 0), spawnPoint[index].rotation);
        yield return new WaitForSeconds(1f);
        Destroy(line);
        yield return new WaitForSeconds(0.5f);
        Instantiate(meteor, spawnPoint[index].position, spawnPoint[index].rotation);
    }
    public IEnumerator IceLance() // 얼음창 생성
    {
        int randomSpawn = Random.Range(0, 5);
        GameObject safe = Instantiate(safeZone, spawnPoint[randomSpawn].position + new Vector3(0, -6, 0), spawnPoint[randomSpawn].rotation);
        yield return new WaitForSeconds(1f);
        Destroy(safe);
        for (int index = 0; index < 5; index++)
        {
            if (index == randomSpawn) continue;
            Instantiate(iceLance, spawnPoint[index].position, spawnPoint[index].rotation * Quaternion.Euler(0, 0, 90));
        }
